...
/**
 * analyze-patterns.mjs — Rejection Pattern Detector for sur9e
 *
...
function classifyOutcome(status) {
  ...
  // SKIP was merged into Discarded in 2026-05; skip alias still mapped to negative
  // for backwards-compat with old tracker rows.
  if (['rejected', 'discarded', 'skip'].includes(s)) return 'negative';
  return 'pending'; // screened, evaluated
}
...
// --- Frontmatter parsing (new report format) ---
// Lightweight YAML-frontmatter reader: flat `key: value` pairs plus the nested
// `score_breakdown:` block. Avoids a yaml dependency for this CLI.
function parseFrontmatter(content) {
  ...
  for (const raw of m[1].split('\n')) {
    ...
    if (/^score_breakdown:\s*$/.test(line)) {
      inSb = true;
      continue;
    }
    if (inSb) {
      const sbm = line.match(/^\s+([a-z_]+):\s*([\d.]+)/i);
      if (sbm) {
        sb[sbm[1]] = Number.parseFloat(sbm[2]);
        continue;
      }
      if (!/^\s/.test(line)) inSb = false; // dedented → block ended
    }
...
// Map frontmatter onto the report shape the downstream analysis expects.
function applyFrontmatter(report, fm) {
  report.archetype = fm.archetype_short || fm.archetype || null;
  report.seniority = fm.seniority || null;
  report.remote = fm.work_mode || fm.remote || null;
  report.comp = fm.comp || null;
  const sb = fm.score_breakdown || {};
  report.scores.cvMatch = sb.cv_match ?? null;
  report.scores.seniority = sb.seniority ?? null;
  report.scores.comp = sb.compensation ?? null;
  report.scores.domain = sb.domain ?? null;
  report.scores.geo = sb.geo ?? null;
  report.scores.legitimacy = sb.legitimacy ?? null;
  report.scores.global = fm.score ? Number.parseFloat(fm.score) : null;
}
...
function parseReport(reportPath) {
  ...
  // New format: header fields + scores live in YAML frontmatter. Read those
  // first; fall back to the legacy body-table regexes only for old reports.
  const fm = parseFrontmatter(content);
  if (fm) applyFrontmatter(report, fm);

  // Legacy body-table extraction (pre-frontmatter reports only).
  if (!fm) {
    // Strip bold markers for easier matching
    const plain = content.replace(/\*\*/g, '');

    // Extract the role-summary table (works with both EN and ES headers)
...
function analyze() {
  ...
  for (const e of enriched) {
    ...
    if (!archetypeMap.has(arch))
      archetypeMap.set(arch, { total: 0, positive: 0, negative: 0, self_filtered: 0, pending: 0 });
  ...
  for (const e of enriched) {
    ...
    if (!remoteMap.has(policy))
      remoteMap.set(policy, { total: 0, positive: 0, negative: 0, self_filtered: 0, pending: 0 });
    const entry = remoteMap.get(policy);
    entry.total++;
    entry[e.outcome]++;
  }
  const remotePolicy = [...remoteMap.entries()]
    .map(([policy, data]) => ({
      policy,
      ...data,
      conversionRate: data.total > 0 ? Math.round((data.positive / data.total) * 100) : 0,
  ...
  for (const e of enriched) {
    ...
    if (!sizeMap.has(size))
      sizeMap.set(size, { total: 0, positive: 0, negative: 0, self_filtered: 0, pending: 0 });
    const entry = sizeMap.get(size);
    entry.total++;
    entry[e.outcome]++;
  }
  const companySizeBreakdown = [...sizeMap.entries()]
    .map(([size, data]) => ({
      size,
      ...data,
      conversionRate: data.total > 0 ? Math.round((data.positive / data.total) * 100) : 0,
  ...
  if (geoBlocker && geoBlocker.percentage >= 20) {
    recommendations.push({
      action: `Tighten location filters in profile.yml (search.locations) -- ${geoBlocker.percentage}% of applications hit a geo-restriction blocker`,
...
function printSummary(result) {
  ...
  console.log(
    `  ${metadata.total} applications (${metadata.dateRange.from} to ${metadata.dateRange.to})`,
  );
  ...
  for (const [group, stats] of Object.entries(scoreComparison)) {
    if (stats.count > 0) {
      console.log(
        `  ${group.padEnd(15)} avg ${stats.avg}/5  (${stats.count} entries, range ${stats.min}-${stats.max})`,
      );
    }
  }
  ...
  if (blockerAnalysis.length > 0) {
    ...
    for (const b of blockerAnalysis) {
      console.log(
        `  ${b.blocker.padEnd(20)} ${String(b.frequency).padStart(2)}x (${b.percentage}% of all)`,
      );
    }
  }
  ...
  for (const r of remotePolicy) {
    console.log(
      `  ${r.policy.padEnd(20)} ${String(r.total).padStart(2)} total, ${r.positive} positive (${r.conversionRate}%)`,
    );
  }
