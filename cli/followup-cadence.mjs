...
/**
 * followup-cadence.mjs — Follow-up Cadence Tracker for sur9e
 *
 ...
 *      node followup-cadence.mjs --num 1272  (scope to a single offer)
 */
...
// --num <N> scopes the dashboard / drafts to a single offer. Used by the
// report toolbar's "Follow up" CLI handoff (one offer at a time).
const numIdx = args.indexOf('--num');
const FILTER_NUM = numIdx !== -1 ? parseInt(args[numIdx + 1]) : null;
...
function computeNextFollowupDate(status, appDate, lastFollowupDate, followupCount) {
  ...
  if (status === 'interview') {
    if (lastFollowupDate) return addDays(parseDate(lastFollowupDate), CADENCE.interview_thankyou);
...
function analyze() {
  ...
  for (const app of apps) {
    ...
    if (FILTER_NUM !== null && Number.isInteger(FILTER_NUM) && app.num !== FILTER_NUM) continue;
