...
3. Each worker evaluates the offer, writes a report to `artifacts/reports/`, generates a PDF to `artifacts/output/`, and writes a tracker TSV to `tracker-additions/`.
...
Workers write one TSV per offer to `batch/tracker-additions/`. The merge script (`npm run tracker:merge`) handles:
...
Run `npm run tracker:merge` manually if you need to merge outside of a batch run.
