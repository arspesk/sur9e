...
# sur9e batch runner — standalone orchestrator for evaluation workers.
# Reads batch-input.tsv, delegates each offer to a worker built through the
# provider layer (claude / codex / opencode — whatever 'evaluate' resolves
# to in Settings → Models), tracks state in batch-state.tsv for resumability.
...
usage() {
  cat <<'USAGE'
sur9e batch runner — process job offers in batch via evaluation workers.
Uses whatever provider + model the 'evaluate' mode resolves to
(Settings → Models): claude, codex, or opencode.
...
check_prerequisites() {
  ...
  # The provider CLI (claude / codex / opencode) is checked in resolve_runtime
  # below, once we know which provider 'evaluate' resolves to.

  if ! command -v npx &>/dev/null; then
    echo "ERROR: 'npx' not found in PATH (needed for the provider resolver via tsx)."
    exit 1
  fi
...
# Resolve provider + model for the 'evaluate' mode via the provider
# registry. We invoke the tsx-backed cli/resolve-mode.mjs helper
# with --shell so it emits bash-eval-safe assignments:
#
#     RESOLVED_PROVIDER='claude'
#     RESOLVED_MODEL='claude-sonnet-4-6'
#     RESOLVED_FROM='mode_setting'
#
# Called once at startup (same model for every worker in this run) and the
# result is reused by process_offer below. Works on any provider: process_offer
# passes the per-offer system prompt via build-claude-cmd.mjs's
# --append-system-prompt-file, and that shim inlines the system prompt into the
# user prompt for non-Claude providers (Codex / OpenCode have no equivalent
# flag), so every adapter gets a shape it accepts.
resolve_runtime() {
  local resolver_out
  if ! resolver_out=$(cd "$PROJECT_DIR" && npx tsx --conditions=react-server cli/resolve-mode.mjs evaluate --shell 2>&1); then
    echo "ERROR: cli/resolve-mode.mjs failed:" >&2
    echo "$resolver_out" >&2
    exit 1
  fi
  # eval the assignments. Values are regex-validated upstream in
  # ProviderModelRef.parse() so they can't carry shell-special characters,
  # but resolve-mode.mjs single-quotes them defensively anyway.
  eval "$resolver_out"

  # The provider's CLI binary shares its name (claude / codex / opencode).
  if ! command -v "${RESOLVED_PROVIDER:-}" &>/dev/null; then
    echo "ERROR: evaluate mode resolved to provider '${RESOLVED_PROVIDER:-(unset)}', but its CLI" >&2
    echo "       ('${RESOLVED_PROVIDER:-(unset)}') is not in PATH. Install it, or re-point 'evaluate'" >&2
    echo "       in Settings → Models to a provider whose CLI is installed." >&2
    exit 1
  fi
}
...
process_offer() {
  ...
  local prompt
  prompt="Process this job offer. Run the full pipeline: evaluation + report .md + PDF + tracker line."
  ...
  # Escape sed delimiter characters in variables to prevent substitution
  # breakage. `&` must be escaped too — in a sed replacement it expands to
  # the matched pattern, corrupting any URL with query parameters.
  ...
  # Build the worker invocation through the provider layer.
  # `cli/build-claude-cmd.mjs` resolves the runtime AND constructs
  # the full eval-safe command line — same model id we already resolved at
  # startup (the shim re-resolves; the result is identical because the
  # config hasn't changed mid-run). We pass the user prompt via --prompt
  # rather than --prompt-file because batch-runner builds the prompt
  # inline as a shell string and a tmp file would just add cleanup risk.
  #
  # The per-offer system prompt goes through --append-system-prompt-file. For
  # Claude that maps to the native flag; for Codex / OpenCode the shim inlines
  # it into the user prompt (neither CLI has an equivalent flag), so this same
  # call works whatever 'evaluate' resolved to.
  local worker_cmd
  if ! worker_cmd=$(cd "$PROJECT_DIR" && npx tsx --conditions=react-server cli/build-claude-cmd.mjs evaluate \
        --prompt "$prompt" \
        --output-format text \
        --no-pipe \
        --append-system-prompt-file "$resolved_prompt" 2>&1); then
    echo "ERROR: build-claude-cmd.mjs failed for offer #$id:" >&2
    echo "$worker_cmd" >&2
    rm -f "$resolved_prompt"
    return 1
  fi

  # Redirect lives OUTSIDE the eval'd string so it applies to the spawned
  # claude process AND any stderr from the bash -c wrapper itself.
  local exit_code=0
  eval "$worker_cmd" > "$log_file" 2>&1 || exit_code=$?
   ...
   score_match=$(sed -nE 's/.*"score":[[:space:]]*([0-9.]+).*/\1/p' "$log_file" 2>/dev/null | head -1 || true)
    ...
    if [[ "$score" != "-" && -n "$score" ]] && (( $(echo "$MIN_SCORE > 0" | bc -l) )); then
      if (( $(echo "$score < $MIN_SCORE" | bc -l) )); then
        ...
        # `return`, not `continue`: process_offer is a function, not a loop
        # body. On bash >= 4.4 `continue` would not reach the caller's loop —
        # it warns and falls through, overwriting "skipped" with "completed".
        return 0
      fi
    fi
    ...
    retries=$((retries + 1))
    local error_msg
    # Flatten newlines AND tabs/CRs — a stray tab in the error field would
    # split the tab-delimited state row and corrupt the retry counter.
    error_msg=$(tail -5 "$log_file" 2>/dev/null | tr '\n\t\r' '   ' | cut -c1-200 || echo "Unknown error (exit code $exit_code)")
...
merge_tracker() {
  ...
  node "$PROJECT_DIR/cli/merge-tracker.mjs"
  echo ""
  echo "=== Verifying pipeline integrity ==="
  node "$PROJECT_DIR/cli/verify-pipeline.mjs" || echo "⚠️  Verification found issues (see above)"
}
...
main() {
  ...
  echo "=== sur9e batch runner ==="
  echo "Parallel: $PARALLEL | Max retries: $MAX_RETRIES"
  echo "Provider: $RESOLVED_PROVIDER | Model: $RESOLVED_MODEL (resolved from $RESOLVED_FROM)"
