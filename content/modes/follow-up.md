...
- `artifacts/reports/` — Evaluation reports (for context in drafts)
- `inputs/personalization/profile.yml` — User profile (name, identity)
- `inputs/personalization/cv.md` — CV for proof points in drafts

## Step 0 — Direct invocation (with offer num)

If invoked as `/sur9e followup <num>` where `<num>` is a positive integer:

1. Run the cadence script scoped to that single offer:

   ```bash
   node src/cli/followup-cadence.mjs --num <num>
   ```

2. Skip the dashboard (Step 2) since there's only one entry. Jump to Step 3 (Generate follow-up draft) for the offer if it's `urgent` or `overdue`. If it's `waiting`, tell the user when the next follow-up is due and stop. If it's `cold` (2+ follow-ups already sent), suggest closing per Step 3's "Cold Application" branch.
3. Steps 4–6 (present draft / record / summary) work the same — just scoped to the one offer.

If invoked without a num (just `/sur9e followup`), proceed from Step 1 as today (fleet-wide dashboard).
...
```bash
node src/cli/followup-cadence.mjs
```
...
> "No active applications to follow up on. Apply to some roles first with `/sur9e` and come back when they're aging."
...
2. Read `inputs/personalization/cv.md` for proof points
3. Read `inputs/personalization/profile.yml` for candidate name and identity
...
2. **Sentence 2:** One concrete value-add from the report's CV Match section or a proof point from inputs/personalization/cv.md. Quantify if possible.
...
- Reference something specific to THAT company (from the report summary)
- Keep under 150 words
- Include a subject line
- Use the candidate's name from `inputs/personalization/profile.yml`
...
- Suggest the user run `/sur9e contact {company}` to find the right person first
...
> - Trying a different contact via `/sur9e contact`
...
**To:** {email or "No contact found — run `/sur9e contact` first"}
