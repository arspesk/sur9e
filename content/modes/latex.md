...
2. Read `inputs/personalization/profile.yml` for candidate identity and contact info
...
The template at `content/templates/cv-template.tex` uses `{{PLACEHOLDER}}` syntax:

| Placeholder            | Source                                                                                                                                                                                            |
| ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `{{NAME}}`             | `inputs/personalization/profile.yml → candidate.full_name`                                                                                                                                        |
| `{{CONTACT_LINE}}`     | Phone / City, State / Visa status — built from inputs/personalization/profile.yml                                                                                                                 |
| `{{EMAIL_URL}}`        | Raw email for `mailto:` URL — must not be LaTeX-escaped (from inputs/personalization/profile.yml)                                                                                                 |
| `{{EMAIL_DISPLAY}}`    | Escaped email for display text — LaTeX-special chars like `_` must be escaped, e.g. `first\_name@example.com`                                                                                     |
| `{{LINKEDIN_URL}}`     | Full URL with scheme for `\href{}`: e.g. `https://linkedin.com/in/username`. If `inputs/personalization/profile.yml` stores a bare host+path (no scheme), prepend `https://` before substitution. |
| `{{LINKEDIN_DISPLAY}}` | Display text only (no scheme): `linkedin.com/in/username`                                                                                                                                         |
| `{{GITHUB_URL}}`       | Full URL with scheme for `\href{}`: e.g. `https://github.com/username`. If `inputs/personalization/profile.yml` stores a bare host+path, prepend `https://`.                                      |
| `{{GITHUB_DISPLAY}}`   | Display text only (no scheme): `github.com/username`                                                                                                                                              |
| `{{EDUCATION}}`        | LaTeX `\resumeSubheading` blocks from inputs/personalization/cv.md Education section                                                                                                              |
...
| `{{SKILLS}}`           | LaTeX `\textbf{Category}{: items}` lines from inputs/personalization/cv.md Technical Skills                                                                                                       |
...
**Exception for URLs:** Do NOT escape text inside `\href{URL}{...}` first arguments. The URL must remain raw (or RFC 3986 percent-encoded). Only escape the _display text_ (second argument). For example:
...
Same ethical rules as `content/modes/tailor-cv.md`:
