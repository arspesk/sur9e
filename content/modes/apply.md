...
**Never use a headless Playwright session for apply.** The candidate must click the final Submit themselves (sur9e never auto-submits), and a headless browser has no window for them to do it in — a headless run only produces a filled form nobody can send, and its state can't transfer to the candidate's own browser. Before relying on the Playwright fallback, confirm it is headed: check `navigator.userAgent` and treat any match of `Headless` (e.g. `HeadlessChrome`) as headless. If the only available Playwright is headless, do NOT use it — fall back to **manual fill in the candidate's own browser** instead: open the form there, reveal the tailored CV + cover-letter PDFs in their file manager, and hand them a copy-paste checklist of every field value so they fill, upload, and submit. Re-enable the Playwright fallback only once it is reconfigured to run headed.
...
Draft an answer for every **real** free-text question on the form. Pull dropdown, yes/no, and self-identification values from `inputs/personalization/profile.yml` where known — `apply_answers` is an ordered list of `{question, answer}` pairs covering standing answers for every recurring form question (work authorization, visa sponsorship, employer, title, school, degree, timezone, opt-ins, self-identification, notice period, and the like). Match each incoming form field to the closest stored `question` (fuzzy/semantic match is fine) and use its `answer`. Ignore entries whose `answer` is empty — treat them as "not provided". Ask only for questions none of the stored answers cover, and on a "save for future use" reply record the new answer as a new `{question, answer}` entry in `apply_answers`.

**Never fabricate sensitive answers.** For legal, work-authorization, visa/sponsorship, salary/compensation, demographic, disability, veteran, criminal-background, security-clearance, relocation, and voluntary self-identification fields, use ONLY a value that is in `profile.yml` → `apply_answers` or unambiguously visible in context. If the answer is not there, do NOT guess or pattern-fill a plausible value — leave it blank, flag it as **needs candidate confirmation**, and ask the candidate the single safest clarifying question. A wrong answer on a legal or EEO field is worse than an unanswered one.
