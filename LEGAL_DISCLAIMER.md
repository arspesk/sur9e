...
`sur9e` is an open-source toolkit: Markdown prompts for AI coding agents, Node.js and Python scripts, and a local web UI (Next.js) that runs entirely on your machine. It is strictly a local execution tool. The maintainers do not host, deploy, or operate an AI system, nor do we provide API access to Large Language Models (LLMs).
...
## 4. Third-Party Platforms & Job Scanning

sur9e discovers new postings by scanning **public LinkedIn job listings** via [JobSpy](https://github.com/cullenwatson/JobSpy) (MIT license, by Cullen Watson). Scanning runs unauthenticated, on your own machine, against publicly visible listings. No LinkedIn account is required. **We strongly recommend that you do NOT log in to LinkedIn in any browser session used by sur9e** — automated access on an authenticated session can trigger account restrictions or bans, and that risk is solely yours.

When processing job URLs you add, sur9e also fetches job-description text from public career-portal pages (Greenhouse, Lever, Ashby, Workday, and others).

- Users must comply with the Terms of Service of every platform they interact with.
- Do not modify the tool to access non-public or authenticated data, or to scrape platforms beyond their publicly visible listings.
...
- The maintainer rejects contributions that facilitate ToS violations.

## 5. Acceptable Use

sur9e is designed to help individuals make better career decisions, not to automate away human judgment. Acceptable use includes:
...
- Scanning non-public or authenticated data, or modifying the tool to do so
