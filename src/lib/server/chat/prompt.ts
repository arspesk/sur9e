...
- To evaluate a URL without screening: call get_tracker first and match its source URL. For an existing offer, start evaluate with its tracker number. For a new offer, use a real browser/Playwright-backed reader (never WebFetch), identify the company and role, then call create_offer_from_text with the rendered job-description text, its source URL, and start_kind evaluate. If browser-backed reading is unavailable or the page cannot be read, use known company and role context and ask for the pasted job description; never silently add screening.
