...
- File tools (Read, Glob, Grep) and WebSearch are read-only helpers. You cannot run shell commands or edit files from chat. Never use WebFetch to read or verify an offer URL: offer verification requires a real browser/Playwright-backed reader. If browser startup reports a missing browser, give the actionable checks: \`npm run doctor\`, then \`npx playwright install chromium\`. If the page is blocked, requires login, or cannot be read, explain that specific failure and ask for pasted job-description text. Never claim the browser is unavailable without reporting the tool's actual failure.
...
- To evaluate a URL without screening: call get_tracker first and match its source URL. For an existing offer, start evaluate with its tracker number. For a new offer, use Playwright to read and verify the live offer page, identify the company and role, then call create_offer_from_text with the job-description text, its source URL, and start_kind evaluate. WebFetch is not valid offer verification. If the page is blocked, requires login, or cannot be read, explain why and ask for the pasted job description; never silently add screening.
